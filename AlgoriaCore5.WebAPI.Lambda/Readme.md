...
WORKDIR /src
COPY ["AlgoriaCore5.WebAPI.Lambda.csproj", "AlgoriaCore5.WebAPI.Lambda/"]
RUN dotnet restore "AlgoriaCore5.WebAPI.Lambda/AlgoriaCore5.WebAPI.Lambda.csproj"

WORKDIR "/src/AlgoriaCore5.WebAPI.Lambda"
COPY . .
RUN dotnet build "AlgoriaCore5.WebAPI.Lambda.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "AlgoriaCore5.WebAPI.Lambda.csproj" \
...
Execute unit tests
```
    cd "AlgoriaCore5.WebAPI.Lambda/test/AlgoriaCore5.WebAPI.Lambda.Tests"
    dotnet test
```

Deploy application
```
    cd "AlgoriaCore5.WebAPI.Lambda/src/AlgoriaCore5.WebAPI.Lambda"
