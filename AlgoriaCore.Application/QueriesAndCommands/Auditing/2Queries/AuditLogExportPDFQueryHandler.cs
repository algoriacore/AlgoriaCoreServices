...
{
    public class AuditLogExportPDFQueryHandler : BaseCoreClass, IRequestHandler<AuditLogExportPDFQuery, FileDto>
    {
        ...
        public AuditLogExportPDFQueryHandler(
        ...
        public async Task<FileDto> Handle(AuditLogExportPDFQuery request, CancellationToken cancellationToken)
        {
            ...
            byte[] bytes = await _pdfService.ExportView(L("AuditLogs"), ll, columns, await GetViewFilters(request));
            return new FileDto
            {
                FileName = "ViewAuditLogs.pdf",
                FileType = "PDF",
                FileBase64 = Convert.ToBase64String(bytes)
            };
        }

        private async Task<List<IViewFilter>> GetViewFilters(AuditLogExportPDFQuery query)
        {
            List<IViewFilter> filters = new List<IViewFilter>();
            string dateRangeStr = query.StartDate.ToShortDateString() + "-" + query.EndDate.ToShortDateString();
